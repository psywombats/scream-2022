...
public class TileImporter : AssetPostprocessor {
    ...
    public void OnPreprocessTexture() {
        ...
        for (int y = 0; y < textureSize.y / Map.PxPerTile; y += 1) {
            for (int x = 0; x < textureSize.x / Map.PxPerTile; x += 1) {
                int index = y * (textureSize.y / Map.PxPerTile) + x;
                SpriteMetaData data = new SpriteMetaData();
                data.rect = new Rect(
                    x * Map.PxPerTile,
                    (textureSize.y / Map.PxPerTile - y - 1) * Map.PxPerTile,
                    Map.PxPerTile,
                    Map.PxPerTile);
    ...
    public static void CreatePalette(string assetPath) {
        ...
        if (palette == null) {
            ...
            grid.cellSize = new Vector3(Map.UnitsPerTile, Map.UnitsPerTile, 0.0f);
    ...
    public static void PopulatePalette(string assetPath) {
        ...
        for (int y = 0; y < tex.height / Map.PxPerTile; y += 1) {
            for (int x = 0; x < tex.width / Map.PxPerTile; x += 1) {
                Vector3Int pos = new Vector3Int(x, (tex.height / Map.PxPerTile - y - 1), 0);
